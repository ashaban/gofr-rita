...
if (cluster.isMaster) {
  ...
  db.once('open', () => {
    ...
    async.each(dataSources, (datasource, nxt) => {
      ...
      const levelData = {
        ...
        level6: 'Village',
        level7: 'null',
        clientId: 'afaf989d-993d-45dc-aafa-34e50d15e5fb',
        userID: '5dd2b3a0064c5303fe0bcb4c',
      };
      mongo.saveLevelMapping(levelData, 'Rita5dd2b3a0064c5303fe0bcb4c', () => {

      });
      mongo.saveLevelMapping(levelData, 'hfr', () => {

      });
      winston.info('Adding data source pairs');
      const pairs = [{
        source1: {
          _id: '606041264e32c53960b0a029',
          userID: {
            _id: '5dd2b3a0064c5303fe0bcb4c',
            userName: 'root@gofr.org',
          },
  ...
  app.get('/getLevelData/:source/:sourceOwner/:level', (req, res) => {
    ...
    let db;
    if (req.params.source === 'hfr') {
      db = 'hfr';
    } else {
      db = mixin.toTitleCase(req.params.source + sourceOwner);
    }
  ...
  app.get('/hierarchy', (req, res) => {
    ...
    if (!source) {
      ...
      if (source === 'hfr') {
        ...
        db = mixin.toTitleCase(source + sourceOwner);
      }
  ...
  app.get('/reconcile', (req, res) => {
    ...
    if (!source2LimitOrgId) {
      source2LimitOrgId = mixin.getTopOrgId('hfr');
    }
    let {
      parentConstraint,
    } = req.query;
